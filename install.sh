...
create_directories() {
    ...
    mkdir -p /var/lib/vps-panel/data
...
generate_env() {
    ...
    cat > "${ENV_FILE}" <<EOF
# VPS Panel Configuration - Generated by installer on $(date -u +"%Y-%m-%dT%H:%M:%SZ")
...
DOCKER_DATA_DIR=/var/lib/vps-panel/data
...
# Branding
BRAND_NAME=VPS Panel
BRAND_COLOR=#6366f1
BRAND_FOOTER=Powered by VPS Panel
EOF
...
prompt_admin() {
    ...
    if [[ ! -t 0 ]]; then
        ...
        read -rp "Admin email [admin@vps-panel.local]: " ADMIN_EMAIL
        ADMIN_EMAIL="${ADMIN_EMAIL:-admin@vps-panel.local}"
...
[Unit]
Description=VPS Panel - VPS Management Service
...
ReadWritePaths=${INSTALL_DIR} /var/lib/vps-panel /tmp
PrivateTmp=true
...
start_systemd() {
    ...
    echo "[INFO] VPS Panel started via systemd."
}

start_screen() {
    if screen -list 2>/dev/null | grep -q "vps-panel"; then
        echo "[INFO] Screen session 'vps-panel' already running."
        return
    fi
    screen -dmS vps-panel bash "\${INSTALL_DIR}/run.sh"
    echo "[INFO] VPS Panel started in screen session."
}
...
stop_systemd() {
    if systemctl is-active --quiet ${SERVICE_NAME} 2>/dev/null; then
        ...
        echo "[INFO] VPS Panel stopped via systemd."
    fi
}
...
stop_screen() {
    if screen -list 2>/dev/null | grep -q "vps-panel"; then
        ...
        echo "[INFO] VPS Panel screen session terminated."
    fi
}
...
create_admin_user() {
    ...
    local ADMIN_EMAIL ADMIN_USERNAME ADMIN_PASSWORD
    ADMIN_EMAIL=$(grep -E "^ADMIN_EMAIL=" "${ENV_FILE}" | cut -d= -f2- || echo "admin@vps-panel.local")
    ...
    # Try to create admin via the application's Python script
...
asyncio.run(create())
" 2>&1 || warn "Admin user creation via API failed. You may need to create it manually."
    fi
}
...
print_summary() {
    ...
    echo ""
    echo -e "${CYAN}============================================================${NC}"
    echo -e "${CYAN}   VPS Panel - Installation Complete!${NC}"
    echo -e "${CYAN}============================================================${NC}"
    echo ""
